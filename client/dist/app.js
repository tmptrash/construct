...
class Observer {
    ...
    on (event, handler) {
        if (typeof(this._handlers[event]) === 'undefined') {
            __WEBPACK_IMPORTED_MODULE_0__Console__["a" /* default */].warn('Invalid event id. Possibly Observer was created with "maxIndex" parameter, which is smaller then "event" id.');
            return;
        }
