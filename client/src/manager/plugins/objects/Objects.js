...
class Objects extends Dots {
    ...
    constructor(manager) {
        super(manager, OConfig, {
            ...
            colorCb    : (    ) => COLOR_RGB,
            checkMin   : (val ) => val + manager.sharedObj.orgEnergy + manager.sharedObj.energy < OConfig.minValue,
            setCb      : (x, y) => manager.positions[x][y] = COLOR_INDEX
        });
        this._energy    = 0;
        this._sharedObj = manager.sharedObj;
        this._sharedObj.objectsEnergy = 0;
    }
    ...
    onLoop(counter, stamp, sharedObj) {
        ...
        sharedObj.objectsEnergy = this._energy;
    }
}
